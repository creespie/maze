def file_parser(filename: str) -> tuple[dict, bool]:
    ...
    keys_set = {"WIDTH", "HEIGHT", "ENTRY", "EXIT", "OUTPUT_FILE", "PERFECT"}
    ...
    try:
                ...
                if line == "" or line[0] == "#":
                    ...
                    if parsed_line[0] == "WIDTH":
                        try:
                            ...
                            raise ValueError(
                                "WIDTH must be submitted in the format 'WIDTH=number'"
                            )
                        ...
                        try:
                            ...
                            raise ValueError(
                                "HEIGHT must be submitted in the format 'HEIGHT=number'"
                            )

                    elif parsed_line[0] == "ENTRY":
                        try:
                            parsed_line[1] = [
                                int(n.strip()) for n in parsed_line[1].split(",")
                            ]

                            if len(parsed_line[1]) != 2:
                                raise TypeError(
                                    "ENTRY value must be formatted as number,number"
                                )
                        ...
                        try:
                            ...
                            if len(parsed_line[1]) != 2:
                                raise TypeError(
                                    "EXIT value must be formatted as number,number"
                                )
                            elif parsed_line[1][0] < 0 or parsed_line[1][1] < 0:
                                raise TypeError("EXIT values can't be negatives")
                            else:
                                configuration[parsed_line[0]] = (
                                    parsed_line[1][0],
                                    parsed_line[1][1],
                        ...
                        try:
                            ...
                            raise ValueError(
                                "SEED must be formatted in the format 'SEED=number'"
                            )
        ...
        if keys_set.issubset(set(configuration.keys())):
            ...
            raise ValueError(
                f"Missing informations about {keys_set.difference(configuration)}"
            )
