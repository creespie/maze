        ...
        with open(filename) as file:
            ...
            for line in actual_text:
                if line == "" or line[0] == "#":
                    ...
                    if len(
                        parsed_line) != 2 or parsed_line[0] not in keys_set:
                        ...
                        if parsed_line[0] == "WIDTH":
                            try:
                                ...
                                if parsed_line[1] < 1: 
                                    ...
                                    raise TypeError("WIDTH was submitted more"
                                                    " than once")
                                else:
                                    configuration[
                                    parsed_line[0]] = parsed_line[1]
                            except ValueError:
                                raise ValueError("WIDTH must be submitted in "
                                                 "the format 'WIDTH=number'")
                            ...
                            try:
                                ...
                                if parsed_line[1] < 1: 
                                    ...
                                    raise TypeError("HEIGHT was submitted"
                                                    " more than once")
                                else:
                                    configuration[
                                    parsed_line[0]] = parsed_line[1]
                            except ValueError:
                                raise ValueError("HEIGHT must be submitted in"
                                                 " the format 'HEIGHT=number'"
                                                 )
                        elif parsed_line[0] == "ENTRY":
                            try:
                                parsed_line[1] = [int(
                                    n.strip()) for n in parsed_line[
                                        1].split(",")]
                                if len(parsed_line[1]) != 2:
                                    raise TypeError("ENTRY value must be form"
                                                    "atted as number,number")
                                elif parsed_line[1][0] < 0 or parsed_line[
                                    1][1] < 0:
                                    raise TypeError("ENTRY values can't "
                                                    "be negatives")
                                elif "ENTRY" in configuration.keys():
                                    raise TypeError("ENTRY was submitted "
                                                    "more than once")
                                else:
                                    configuration[parsed_line[0]] = (
                                        parsed_line[1][0], parsed_line[1][1])
                            except ValueError:
                                raise ValueError("ENTRY must be submitted in "
                                                 "the format "
                                                 "'ENTRY=number,number'")
                            ...
                            try:
                                parsed_line[1] = [int(
                                    n.strip()) for n in parsed_line[
                                        1].split(",")]
                                if len(parsed_line[1]) != 2 or parsed_line[
                                    1][0] < 0 or parsed_line[1][1] < 0:
                                    raise TypeError("Wrong EXIT input for "
                                                    "return _parse_int"
                                                    "(raw, kmat)")
                                elif "EXIT" in configuration.keys():
                                    raise TypeError("EXIT was submitted more"
                                                    " than once")
                                else:
                                    configuration[parsed_line[0]] = (
                                        parsed_line[1][0], parsed_line[1][1])
                            except ValueError:
                                raise ValueError("EXIT must be submitted in "
                                                 "the format "
                                                 "'EXIT=number,number'")
                        elif parsed_line[0] == "OUTPUT_FILE":
                            if "OUTPUT_FILE" in configuration.keys():
                                    raise TypeError("OUTPUT_FILE was subm"
                                                    "itted more than once")
                            try:
                                ...
                                raise ValueError("OUTPUT_FILE must be submit"
                                                 "ted in the format "
                                                 "'OUTPUT_FILE=file' "
                            ...
                            try:
                                if "PERFECT" in configuration.keys():
                                    raise TypeError("PERFECT was submitted "
                                                    "more than once")
                                parsed_line[1] = parsed_line[
                                    1].strip().capitalize()
                                if parsed_line[1] == "True":
                                    ...
                                    raise TypeError("PERFECT value can only "
                                                    "be true or false")
                            except ValueError:
                                raise ValueError("PERFECT must be submitted "
                                                 "in the format "
                                                 "'PERFECT=bool'")
                            ...
                            try:
                                ...
                                if "SEED" in configuration.keys():
                                    raise TypeError("SEED was submitted more "
                                                    "than once")
                                else:
                                    configuration[
                                        parsed_line[0]] = parsed_line[1]
                            except ValueError:
                                raise ValueError("SEED must be formatted in "
                                                 "the format 'SEED=number'")
        if keys_set == set(configuration.keys()):
            return (configuration, True)
        else:
            raise ValueError(f"Missing informations about "
